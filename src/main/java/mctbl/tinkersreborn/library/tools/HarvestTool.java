	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
		...
		if (targetStack != null) {
			...
			if (item instanceof ItemBlock) {
				...
				AxisAlignedBB blockBounds = AxisAlignedBB.getBoundingBox(posX, posY, posZ, posX + 1, posY + 1,
						posZ + 1);
				AxisAlignedBB playerBounds = player.boundingBox;

				if (item instanceof ItemBlock i) {
					// prevent player place a sand then fall with it
					if (i.field_150939_a.getMaterial().blocksMovement() && playerBounds.intersectsWith(blockBounds)) {
						return false;
					}
				}
